...
JSONAdaptor.prototype._save = function(callback) {
  ...
  this.q = this.q || async.queue(function(json, done) {
    fs.writeFile(self._path, json, 'utf8', done)
  }, 1)
  this.q.saturated = function() {
    if (self.q.length > self.q.concurrency * 2) self.q.tasks.shift()
  }
  this.q.push(json, function(err) {
    callback(err)
  })
}
...
JSONAdaptor.prototype.find = function(type, id, callback) {
  var self = this
  this._memoryAdaptor.find.apply(this._memoryAdaptor, arguments)
}

JSONAdaptor.prototype.findMany = function(type, ids, callback) {
  var self = this
  this._memoryAdaptor.findMany.apply(this._memoryAdaptor, arguments)
}
...
JSONAdaptor.prototype.insertMany = function(type, id, syndex, callback) {
...
JSONAdaptor.prototype.updateMany = function(type, id, syndex, callback) {
  return this._markMany.apply(this, arguments)
}
...
JSONAdaptor.prototype.removeMany = function(type, id, syndex, callback) {
  return this._markMany.apply(this, arguments)
}

JSONAdaptor.prototype._mark = function(type, id, syndex, callback) {
  var self = this
  var callback = arguments[arguments.length - 1]
  this._memoryAdaptor._mark(type, id, syndex, function(err) {
    var args = arguments
    self._save(function(saveErr) {
      if (err) return callback(err)
      if (saveErr) return callback(saveErr)
...
JSONAdaptor.prototype._markMany = function(type, ids, syndex, callback) {
  var self = this
  var callback = arguments[arguments.length - 1]
  this._memoryAdaptor._markMany(type, ids, syndex, function(err) {
    var args = arguments
    self._save(function(saveErr) {
      if (err) return callback(err)
      if (saveErr) return callback(saveErr)
