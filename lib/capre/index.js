...
Capre.prototype.flatten = function(type, callback) {
  var self = this
  self.backend.getTypeInfo(type, function(err, info) {
    if (err) return callback(err)
    var syndex = info.syndex
    self.backend.getAboveSyndex(type, 0, function(err, items) {
      if (err) return callback(err)
      self.backend.flush(function(err) {
        if (err) return callback(err)
        self.backend.register(type, function(err) {
          if (err) return callback(err)
          self.backend.insertMany(type, items, syndex, callback)
        })
      })
    })
  })
