...
Master.prototype.register = function(type, callback) {
  ...
  var self = this
  this.backend.getTypeInfo(type, function(err, getTypeInfo) {
    if (getTypeInfo) return callback(new Error('type already exists: ' + type))
...
Master.prototype.getSyndex = function(type, id, callback) {
  ...
  this.backend.getTypeInfo(type, function(err, typeInfo) {
    ...
    if (!typeInfo) return callback(new Error('unknown type: ' + type))
    if (id === null || id === undefined) {
      return callback(null, typeInfo.syndex)
    }
